...
2. **LinkML runtime compatibility.** Resolved. `linkml-runtime.SchemaView` parses `cosmos_sdtm_model.yaml` and `cosmos_bc_model.yaml`; `linkml.validator.validate` returns zero errors on `sdtm_abi.yaml`. Verified 2026-04-22 in the Step 1 graph-walk audit (exploratory notebook removed after Step 2 closure).
3. **xlsx ↔ LinkML equivalence.** New, resolved. All 32 xlsx columns map to LinkML slots (0 unmapped). Reification quad populated on 97.5% of rows / 99.7% of DSSs. ABI xlsx rows and `sdtm_abi.yaml` agree fact-for-fact on all 6 variables (`VSTESTCD`, `VSTEST`, `VSORRES`, `VSSTRESC`, `VSSTRESN`, `VSDTC`). Verified 2026-04-22 in the Step 1 xlsx-source audit (exploratory notebook removed after Step 2 closure). Implication: the xlsx is graph-equivalent to YAML at VLM-row grain; schema-driven flattening is tractable against the xlsx today.
