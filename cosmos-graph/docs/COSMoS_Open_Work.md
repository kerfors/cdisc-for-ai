...
- **BC-side validation in `30_validate_graph.ipynb`.** The five BC-side projection sheets (`BC_Parents`, `BC_Categories`, `Categories`, `Coding`, `DataElementConcepts`) have no hard validation in notebook 30. Referential integrity (every `bc_id` resolves against `BC`, every `parent_bc_id` resolves, `BC_Categories` closes against the `Categories` vocabulary) is asserted as inline build-time prints in notebook 10, not promoted to hard checks. **Trigger:** a consumer track depends on these sheets, or any of the inline checks ever reports a non-zero gap.
